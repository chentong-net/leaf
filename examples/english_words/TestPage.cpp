...
EnglishWordsTestMode testModeFromIndex(int index) {
    switch (index) {
        case 0: return EnglishWordsTestMode::ChineseToEnglish;
        case 1: return EnglishWordsTestMode::RussianToEnglish;
        case 2: return EnglishWordsTestMode::EnglishToChinese;
        case 3: return EnglishWordsTestMode::EnglishToRussian;
        case 4: return EnglishWordsTestMode::AudioToChinese;
        case 5: return EnglishWordsTestMode::AudioToRussian;
        default: return EnglishWordsTestMode::AudioToEnglish;
    }
}

int testModeToIndex(EnglishWordsTestMode mode) {
    switch (mode) {
        case EnglishWordsTestMode::ChineseToEnglish: return 0;
        case EnglishWordsTestMode::RussianToEnglish: return 1;
        case EnglishWordsTestMode::EnglishToChinese: return 2;
        case EnglishWordsTestMode::EnglishToRussian: return 3;
        case EnglishWordsTestMode::AudioToChinese: return 4;
        case EnglishWordsTestMode::AudioToRussian: return 5;
        case EnglishWordsTestMode::AudioToEnglish: return 6;
    }
...
std::shared_ptr<TestPage> TestPage::create() {
...
void TestPage::buildUI() {
    ...
    std::weak_ptr<TestPage> weakSelf = std::static_pointer_cast<TestPage>(shared_from_this());
    ...
    m_modeDropdown->setMaxPanelHeight(300.0f);
    m_modeDropdown->setFontSize(15.0f);
    m_modeDropdown->setCornerRadius(20.0f);
    m_modeDropdown->setBorderColor(0xFFD8E4F1);
    m_modeDropdown->setTriggerBackgroundColor(0xFFFFFFFF, 0xFFEAF3FF);
    m_modeDropdown->setPanelBackgroundColor(0xFFFFFFFF);
    m_modeDropdown->setOptionBackgroundColor(0xFFFFFFFF, 0xFFF4F8FC, 0xFFEAF3FF);
    m_modeDropdown->setSelectedTextColor(0xFF3567A3);
    m_modeDropdown->getTriggerButton()->setHeight(56.0f);
    m_modeDropdown->getTriggerButton()->setShadow(0.0f, 8.0f, 20.0f, 0.0f, 0x10233B53);
    m_modeDropdown->setOnSelectionChanged([weakSelf](int index, const std::string&) {
        auto self = weakSelf.lock();
        if (!self) {
            return;
        }
        self->m_selectedMode = testModeFromIndex(index);
    });
    m_modeDropdown->setSelectedIndex(testModeToIndex(m_selectedMode), false);
    root->addChild(m_modeDropdown);
    ...
    for (int index = 0; index < static_cast<int>(m_levels.size()); ++index) {
        ...
        section->bind(
            ...
            [weakSelf](const EnglishWordTopic& topic) {
                ...
                if (auto navigator = self->getNavigator()) {
                    navigator->push(ExamPage::create(topic, self->m_selectedMode));
                }
            }
        );
...
void TestPage::showStatus(const std::string& text) {
    if (!m_content) {
        return;
    }
